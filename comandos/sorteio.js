...
exports.run = async (bot, message, args) => {
    ...
    if (
        ...
        return message.channel.send(
            ` ${message.author}, Você não está fazendo corretamente. (/sorteio tempo + canal + premio)`
        );
    if (!message.member.permissions.has("ADMINISTRATOR"))
        return message.reply(
            " Você não tem permissão para isso."
    ...
    let Embed = new MessageEmbed()
        .setTitle(`SORTEIO`)
        .setThumbnail(icon)
        .setDescription(`**Novo sorteio na área, não vai perder essa né?** \n\n Sorteio feito por: ${message.author} \n\nPrêmio do sorteio: **${prize}** \n\n🕗| Tempo de sorteio: **${tempo}** \n\n Para participar reaja com: <:MegaThonk:805762596547330059>`)
        .setTimestamp(Date.now() + ms(args[0]))
        .setFooter({text: 'O sorteio ira acontecer', iconURL: ''})
        .setColor(`#6400b6`);
    if (!message.member.permissions.has("ADMINISTRATOR"))
        return message.reply(
            " Você não tem permissão para isso."

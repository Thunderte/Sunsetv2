...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    const embed = new Discord.MessageEmbed()//na dm bem obvio
  .setTitle(`Painel Comandos`)
  .setColor([255,182,193])
  .setDescription(`Escolha nos botões qual comando você quer ver!`)

  message.channel.send({embeds: [embed], components: [row], ephemeral: true}).then(escolha => {
      ...
      switch(i.customId){
        ...
        case `normal`:
          const embed4 = new MessageEmbed()
              ...
              message.channel.send({embeds: [embed4], components: [row], ephemeral: true})
              break;
      }
    })
  })
